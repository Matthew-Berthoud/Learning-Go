...
func main() {
	...
	for i := 0; i < 10; i++ {
		total := total + i
		// total = total + i // debugged version
		fmt.Println(total)
	}
	fmt.Println(total)
}

// The bug is that total is getting re-declared in the loop, therefore shadowing its former value
// Removing the colon from the := assignment would solve the problem
